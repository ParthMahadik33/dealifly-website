...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var hidePreloader = function hidePreloader() {
    ...
    window.setTimeout(function () {
      if (preloader.classList.contains("preloader--loaded")) {
  ...
  // Register the escape path before optional animation features initialize. This
  // keeps the full-screen loader from trapping the page if an older browser is
  // missing one of the enhancement APIs used later in this file.
  ...
  if (primaryHeader) {
    ...
    var menuMediaQuery = window.matchMedia(mdScreen);
    var resetMobileMenus = function resetMobileMenus() {
    ...
    if (typeof menuMediaQuery.addEventListener === "function") {
      menuMediaQuery.addEventListener("change", resetMobileMenus);
    } else if (typeof menuMediaQuery.addListener === "function") {
      // Safari versions before MediaQueryList inherited EventTarget.
      menuMediaQuery.addListener(resetMobileMenus);
    }
  }
  ...
  scrollerX.forEach(scrollerXDuplication);
  ...
  // Infinite-scroller CSS assumes the second copy exists from frame one. Build
  // it synchronously so mobile browsers cannot animate into an empty half while
  // waiting for IntersectionObserver (or fail entirely when it is unavailable).
  scrollerY.forEach(scrollerYDuplication);
  ...
  document.addEventListener("click", function (event) {
    ...
    var isInternalPage = destination.origin === window.location.origin && (destination.pathname !== window.location.pathname || destination.search !== window.location.search);
    if (isInternalPage) showPreloader();
  });

  /**
   * Decorative hero video with forward/reverse playback
   */
  var initializeHeroPingPong = function initializeHeroPingPong() {
    var videos = document.querySelectorAll("[data-hero-pingpong]");
    ...
    videos.forEach(function (video) {
      ...
      var isVisible = true;
      var isReady = video.readyState >= 2;
      var cancelReverse = function cancelReverse() {
      ...
      var _reverseStep = function reverseStep(timestamp) {
        if (direction !== "reverse" || !isVisible || document.hidden || reducedMotion.matches) {
          reverseFrame = null;
          return;
        }
        ...
        if (elapsed >= 32) {
          video.currentTime = Math.max(0, video.currentTime - elapsed / 1000 * 0.72);
        ...
        reverseFrame = window.requestAnimationFrame(_reverseStep);
      };
      var playReverse = function playReverse() {
        if (!isReady || reducedMotion.matches) return;
        cancelReverse();
        direction = "reverse";
        previousTimestamp = 0;
        video.pause();
  ...
  /**
   * Accessible controls for the B2B process video
   */
  var initializeVideoPlayers = function initializeVideoPlayers() {
    var players = document.querySelectorAll("[data-video-player]");
    if (!players.length) return;
    players.forEach(function (player) {
      var video = player.querySelector("[data-video-media]");
      var playButton = player.querySelector("[data-video-toggle-play]");
      var muteButton = player.querySelector("[data-video-toggle-mute]");
      if (!video || !playButton || !muteButton) return;
      var playIcon = playButton.querySelector("i");
      var playLabel = playButton.querySelector("span");
      var muteIcon = muteButton.querySelector("i");
      var muteLabel = muteButton.querySelector("span");
      var syncPlayControl = function syncPlayControl() {
        var isPaused = video.paused;
        playButton.setAttribute("aria-label", isPaused ? "Play video" : "Pause video");
        if (playIcon) {
          playIcon.className = isPaused ? "bi bi-play-fill" : "bi bi-pause-fill";
        }
        if (playLabel) playLabel.textContent = isPaused ? "Play" : "Pause";
      };
      var syncMuteControl = function syncMuteControl() {
      ...
      playButton.addEventListener("click", function () {
        if (video.paused) {
          var playRequest = video.play();
          if (playRequest) playRequest["catch"](syncPlayControl);
        } else {
          video.pause();
        }
      });
      ...
      video.addEventListener("play", syncPlayControl);
      video.addEventListener("pause", syncPlayControl);
      video.addEventListener("volumechange", syncMuteControl);
      syncPlayControl();
      syncMuteControl();
    });
  };
  initializeHeroPingPong();
  ...
  mm.add("(min-width: 992px)", function () {
    ...
    if (folderCards.length) {
      ...
      var folderCardImages = document.querySelectorAll(".dealifly-core-service-visual");
      var folderCardRefreshTimer;
      var refreshFolderCardPins = function refreshFolderCardPins() {
        window.clearTimeout(folderCardRefreshTimer);
        folderCardRefreshTimer = window.setTimeout(function () {
          ScrollTrigger.refresh();
        }, 80);
      };
      folderCardImages.forEach(function (image) {
        if (!image.complete) {
          image.addEventListener("load", refreshFolderCardPins, {
            once: true
          });
        }
      });
...
(function () {
  ...
  var statusElement = document.getElementById("form-message");
  if (!statusElement) {
    return;
  }
  var status = new URLSearchParams(window.location.search).get("status");
  var messages = {
    success: "Thank you. Your message has been sent successfully. Our team will get back to you shortly.",
    invalid: "Please complete all required fields with valid information.",
    error: "Your message could not be sent. Please email contact@dealifly.com directly."
  };
  if (!messages[status]) {
    return;
  }
  statusElement.textContent = messages[status];
  statusElement.classList.add(status === "success" ? "success" : "error");
})();